...
**Backward sweep** (HITL offer, beside the roll-forward). What this run landed can make a sibling spec's *absence* claim false — "no adapter exists yet", "the table is not wired". The candidate set is closed: the open siblings under the same parent issue carrying `cascade-depth:roughed-in`, read in **one** call (`gh api repos/{owner}/{repo}/issues/{parent}/sub_issues` on github-issues, the parent's children on linear) and capped by the milestone's R-count — never the whole backlog. Check absence claims only, and verify each by **running the body's own command** (its grep, its test, its probe), not by recalling what this run did. Separate the two kinds: a claim this landing falsified (report it, with the commit) from a claim that was false when written (report it as a spec defect). **Default to not reporting** — a claim that still holds is not mentioned. The repair is rough-in's in-place body refresh, offered to the operator; this run never edits an issue body. An empty sweep is a one-line pass — `backward sweep: N siblings, no absence claim falsified` — never an omitted line.
