...
- `Grep` / `Glob` for the surface; find-references / symbol info / call sites for the blast radius (which tool: `tooling.md` § Code intelligence); documentation-lookup tooling to confirm a library's actual behavior; read-only data/schema inspection when the issue is data-shaped.
