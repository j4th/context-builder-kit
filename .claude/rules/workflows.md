...
[If the project's viability depends on staying within a resource budget, record the discipline here. **Name money and agent-run quota separately** — a paid API tier is a monthly amount, an agent-run quota (subscription usage, a CI-minutes cap, a per-workflow agent ceiling) is a count with a reset period, and a milestone that is free in dollars can still exhaust the quota; each cascade artifact that touches either states which one it spends. The observed shape: every cascade artifact touching paid resources or scope surfaces (1) the monthly cost — default free tier; if non-zero, name the tier and the amount — and (2) the required-vs-aspirational distinction (which deliverables are load-bearing vs nice-to-have). Default to the free tier; a paid resource is named explicitly for the operator's eyes-open approval. Per-milestone cost surfaces live in the framing docs; per-PR cost notes go in the PR body when a paid resource is introduced or changed.]
