...
grep -q 'total list-price cost: \$21.25' <<<"$out" || { echo "expected a \$21.25 total; got:"; echo "$out"; exit 1; }
grep -q '2 of 4 agents priced' <<<"$out" || { echo "the priced/unpriced split is not printed"; echo "$out"; exit 1; }
