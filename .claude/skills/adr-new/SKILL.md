...
**`Extends: ADR-NNNN (Dn, …)`** — the new ADR *adds an obligation beside* a parent clause that stays satisfied as written; the parent is not narrowed and stays `Accepted`. **The disambiguation test:** a child that removes a permitted reading of the parent clause Refines; a child that adds an obligation beside a clause that stays satisfied Extends. The header narrative is the same as a refine's — per clause, what is added and what stays binding — and the asymmetry is the same: no back-pointer on the parent.
