...
**Via MCP:**
1. **Create the repository.** Use the `create_repository` tool. Inputs: name (derived from problem brief, in kebab-case, confirm with user), visibility (ask: public or private — solo defaults to private, team to whatever the org standard is), initialize with README, add a `.gitignore` for the one stack hint the brief gives, **anchored** per `cbk-conventions-reference.md` § .gitignore anchoring (or skip if no signal), and **confirm the licence the way you confirm visibility** — one question: MIT offered as the solo default, the org standard for a team, never a copyleft licence without explicit opt-in, and "none yet — all rights reserved" a valid answer that is recorded, not skipped (`cbk-conventions.md` § Licensing). Seed `LICENSE` and the README § License from the answer.
2. **Push starter `.github/` files.** Commit via the file-creation tool: an issue template for bug reports, an issue template for features, a PR template, a `CODEOWNERS` file (single owner = the user, for solo; ask for team), and one CI workflow stub (`.github/workflows/ci.yml`) that runs an empty job — real CI gets configured in blueprint. The literal bodies live in `references/github-starter-templates.md` — the issue forms (provenance and holding labels applied at creation), `config.yml` (blank issues enabled), the PR template (the `## Review gate` and `## Triage` blocks, empty), `CODEOWNERS`, the CI stub with its promotion traps, `dependabot.yml` with the floor, and the `.gitattributes` counter-line when a lockfile exists.

**Manual (generate instructions inline, user completes in browser tabs):**
3. **Create the label taxonomy — the full set, as an axis structure.** Visit `<repo URL>/labels` (or run `gh label create` per label when a token exists — the same list either way). Delete or rename GitHub's auto-generated defaults (`bug`, `enhancement`, `question`, …) first so they cannot collide, then create every axis: **cascade depth** — `cascade-depth:rough` (the workstream parent blueprint creates), `cascade-depth:framed`, `cascade-depth:roughed-in`; **awaiting cascade work** — `enhancement` (the transient holding marker `/intake` reads; see `cbk-conventions-reference.md` § Contribution intake); **provenance** — `source:github`, plus one `source:<name>` per external channel the project takes reports from; **type** — `bug`, `feature`, `improvement`, `chore`, `documentation`, `tech-debt`; **workstream** — `workstream:<slug>` for every slug the brief already names (blueprint creates the rest when it creates the parent issues; framing creates one for a slug blueprint did not); **meta** — `meta`; **review control** — `skip-claude`, `claude-review-again`, `claude-fast-review`, `claude-deep-review` (read by the auto-review workflow blueprint may emit; harmless when it does not); and the optional **labour** axis — `labour:<lane>` only where the project distinguishes lanes, the default lane unlabelled. Every label a handoff doc or template names must be in this set; a label named nowhere is not created.
...
- **`.gitignore` overreach**: don't generate a 500-line `.gitignore` with every possible language. Pick one stack hint from the brief, generate that anchored (`/target/`, not `target/` — `cbk-conventions-reference.md` § .gitignore anchoring), note that the user can extend later.
