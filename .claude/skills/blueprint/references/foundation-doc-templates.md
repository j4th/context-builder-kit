...
**Exception**: README.md's Documentation table uses markdown links because the README's audience is GitHub's rendered view and the links are the whole point of the table. CLAUDE.md does **not** get one: its references to the other docs are backticked paths, never `@docs/file.md` — an `@path` line is an import that expands its target into every session (the `@` trap note in `templates/claude-md.md`; the mechanism and its citation live in `cbk-conventions.md` § Rule loading and the instruction budget). This exception is about target audience, not preference — only override the inline-code default when there's a real audience reason.
