...
- **Provenance**: one sentence on where the template came from (*"kara-validated revision of the speculative v0 — runs `/simplify` and `pr-review-toolkit:review-pr` pre-PR with a four-class auto-triage, opens draft, hands off to user for ready-flip"*)
...
The template is extracted verbatim — rough-in does not edit, customize, or parameterize it during provisioning. If a user's repo needs a customized `/finish` (kara-specific ADR list, project-specific reviewer agents, branch-naming conventions), they edit the committed file directly after provisioning.
