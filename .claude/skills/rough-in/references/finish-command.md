...
- **Structure**: a numbered list of the template's steps with one-line descriptions each (*"Step 1: Read the issue (+ comments, break-glass check) / Step 2: Parse sections / Step 3: Verify dependencies / Step 4: Idempotency / Step 5: Executable research, then plan-mode gate / Step 6: Create branch + execute / Step 7: Test gate / Step 8: Simplify + triage / Step 9: Review + triage / Step 10: Push + draft PR / Step 11: Hand off"* — roughly a dozen lines total)
...
2. **Dispatch project-local reviewers in parallel** with the toolkit's generic agents. The dispatch roster and its conditions live in exactly one place — `pr-review.md` § Project-local agents — and this step consults it rather than keeping its own copy. Common shape: cross-cutting reviewers (ADR-conformance, logging-discipline, cascade-rule) run unconditionally and return a cheap clean verdict when nothing is in scope; domain reviewers are path-matched against the changed files. If your repo hasn't authored these, the toolkit's generic agents alone are sufficient. Where the project runs the review pass as an orchestrated find-then-verify sweep (see `pr-review.md` § When to invoke), dispatch that instead — direct dispatch is the fallback, and the hand-off records which path ran.
