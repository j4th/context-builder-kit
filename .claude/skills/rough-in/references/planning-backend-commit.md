...
```bash
set -euo pipefail                                                                              # stop at the first failure (§ Partial failure recovery); never run on an empty value
...
n=${url##*/}; [ -n "$n" ] || { echo "issue create returned no URL" >&2; exit 1; }
id=$(gh api "repos/$repo/issues/$n" --jq .id); [ -n "$id" ] || { echo "no id for #$n" >&2; exit 1; }                                                   # the numeric id the link needs (not the number): `sub_issue_id` — "The id of the sub-issue" (`https://docs.github.com/en/rest/issues/sub-issues`, read 2026-09-06)
...
**`linear` planning**: rough-in's sub-sub-issues are created via `mcp__linear__save_issue` with `parentId` = the framing F sub-issue — parent/sub-issue links all the way down (workstream parent issue → F sub-issue → R sub-sub-issue); the planner's native Project-milestones field is deliberately unused by the cascade. Same atomic transition pattern. See `planning-backend-matrix.md` for full operational detail.
