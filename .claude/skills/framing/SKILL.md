...
Additive increment vs. re-framing (pattern C) is a status distinction: re-framing **replaces** a milestone breakdown that didn't survive contact with the code (old frame → `Superseded`); an additive increment **extends** a workstream whose prior milestones are still valid and open (old frame stays `Active`). Reach for additive increment when a gap assessment or a newly-surfaced prerequisite reveals work the prior frame didn't cover *and* the prior frame's milestones are still the right shape. Reach for re-framing when the prior frame's milestones themselves need re-cutting.
