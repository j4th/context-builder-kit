...
- **A fact stated on more than one surface is stated once, with its other locations recorded.** Back-pointers are asymmetric (the immutable source never points forward; the companion points back), a re-check trigger sits at the rail it fires, and closed sets are diffed by the verification block where the tree can. The companion vocabulary — immutable source plus append-only companion — has three instances: ADRs + `docs/adr/corrections.md`, a frozen pre-cascade corpus + its errata, a repo artifact + its knowledge-backend page. See `cbk-conventions.md` § Multi-surface facts.
