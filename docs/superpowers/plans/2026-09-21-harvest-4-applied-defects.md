...
Run (mutation — a target that wired the formatter, undeclared), in a throwaway copy: `cp -r . /tmp/kitcopy && cd /tmp/kitcopy && mkdir -p docs/cbk && touch docs/cbk/scaffold.md` (the project sub-block is gated on that file); append `format-on-edit.sh` to `.claude/rules/cbk-conventions.md` (the kit sub-block's registry-naming check runs before the arm and fires on any registered hook the mutation table does not name); register `format-on-edit.sh` in `settings.json`'s `PostToolUse` with `jq`; run the runner and expect `advisory hook format-on-edit.sh is registered but not declared in ADVISORY_WIRED` as the failure line. Then `ADVISORY_WIRED=format-on-edit.sh bash .claude/workflows/tests/run-verification-block.sh` and expect that line GONE: the run continues past the arm into target-only checks a copied kit tree cannot satisfy (the stub-language literal the plans under `docs/superpowers/` quote, then the unfilled `paths:` placeholder) and exits 1 there — that is the arm passing; a green project sub-block is not reachable from a kit copy. Register the hook a second time and expect `advisory hook format-on-edit.sh is declared wired but registered 2 times`. `cd -; rm -rf /tmp/kitcopy`. (Recorded from a live run, 2026-09-21: as first written this step named neither precondition and expected the positive arm to "pass", which a kit copy cannot show — review sweep, test-coverage dimension.)
